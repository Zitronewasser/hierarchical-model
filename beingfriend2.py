...
from generateData import grades_arr, homogeneous_graph, adj_matrix_coo_ini, num_nodes, degree,seed
import time
...
# a1 = adj_matrix_coo.toarray()
# aa1 = np.zeros((8,8),dtype=int)
# for i1 in range(8):
#     for j1 in range(8):
#         if a1[i1,j1] == 1:
#             aa1[i1,j1] = grades_arr[j1]
# print(aa1)


# 打印邻接矩阵
# print("Dense adj_matrix:\n")
# print(adj_matrix)

# 统计每个节点的度数
# degrees = [degree for _, degree in homogeneous_graph.degree()]
# print("degrees:")
# print(degrees)
...
num_connected_components = nx.number_connected_components(homogeneous_graph)
print("连通子图数量:", num_connected_components)
...
def drop_the_lowest_g(t_coo_matrix, t_row_number):  # 这个函数把接收到的这个行标的朋友里分数最低的那个人drop掉
    ...
    drop_col_index = np.argsort(each_f_grade_list)[
                     -each_f_nonzero_count:]  # 这是一个分数从低高到排序后返回的列索引，只保留倒数each_f_nonzero_count个
...
def adj_to_connected_c(t_coo_matrix):
    # 将COO格式的稀疏矩阵转换为NetworkX图
    G = nx.from_scipy_sparse_array(t_coo_matrix)
    # print(nx.number_connected_components(G))
    # 使用NetworkX的connected_components方法计算连通子图数量
    return nx.number_connected_components(G)
    # # 创建一个无向图
    # t_adj_graph = nx.Graph()
    # # 添加节点
    # t_number_nodes = t_coo_matrix.shape[0]
    # t_adj_graph.add_nodes_from(range(t_number_nodes))
    # # 添加边
    # for i11, j1 in zip(t_coo_matrix.row, t_coo_matrix.col):
    #     t_adj_graph.add_edge(i11, j1)
    # connected_components = list(nx.connected_components(t_adj_graph))
    # num_connected_components = len(connected_components)
    #
    # return num_connected_components


def fun_pre_cc(t_coo_matrix):
    # 先转换成网络
    t_adj_graph = nx.Graph()
    t_number_nodes = t_coo_matrix.shape[0]  # 添加节点
    t_adj_graph.add_nodes_from(range(t_number_nodes))
    for i11, j1 in zip(t_coo_matrix.row, t_coo_matrix.col):  # 添加边
        t_adj_graph.add_edge(i11, j1)

    # 检索第一个值为 9 的元素,返回列索引
    score_nine_indices = np.where((9 < grades_arr) & (grades_arr <= 10))
    first_score_nine_indices = score_nine_indices[0][0]
    score_five_indices = np.where((5 < grades_arr) & (grades_arr <= 6))
    first_score_five_indices = score_five_indices[0][0]
    score_one_indices = np.where((0 < grades_arr) & (grades_arr <= 1))
    first_score_one_indices = score_one_indices[0][0]
    # 计算选定节点的集聚系数
    clustering_coefficient_nine = round(nx.clustering(t_adj_graph, nodes=first_score_nine_indices), 2)
    clustering_coefficient_five = round(nx.clustering(t_adj_graph, nodes=first_score_five_indices), 2)
    clustering_coefficient_one = round(nx.clustering(t_adj_graph, nodes=first_score_one_indices), 2)
    # 计算整个网络的平均集聚系数
    avg_clustering = round(nx.average_clustering(t_adj_graph), 2)
    return clustering_coefficient_nine, clustering_coefficient_five, clustering_coefficient_one, avg_clustering


def coo_to_friend_vector(t_coo_matrix):

    z_t = 0.5  # 赋予值
    mean_score_vector = []
    grades_arr_sum_f_t2 = np.zeros(num_nodes, dtype=float)  # 当场计算标准分数
    for i1 in range(num_nodes):
        count_f = np.count_nonzero(col_index_to_grade(adj_matrix_coo, i1))
        if count_f > 0:
            sum_f = np.sum(col_index_to_grade(t_coo_matrix, i1))
            grades_arr_sum_f_t2[i1] = float(sum_f / count_f)
    grades_arr_sum_f_t2 = np.array(grades_arr_sum_f_t2)
    score_arr_standard = z_t * grades_arr + (1-z_t) * grades_arr_sum_f_t2
    t_arr1d_for_sort_g = np.zeros(num_nodes, dtype=float)
    # print(grades_arr)
    score_nine_indices = np.where((grades_arr >= 8) & (grades_arr < 9))
    self_score = score_arr_standard[score_nine_indices[0]]
    mean_score_vector.append(self_score)
    # for i in range(len(score_nine_indices[0])):
    #
    #     first_score_nine_indices = score_nine_indices[0][i]
    #     for col1 in t_coo_matrix.col[t_coo_matrix.row == first_score_nine_indices]:  # 仿照col_index_to_grade函数
    #         t_arr1d_for_sort_g[col1] = score_arr_standard[col1]  # 此处为读取score_standard里i1的朋友的标准分数
    #     vector_score_friend = t_arr1d_for_sort_g[t_arr1d_for_sort_g != 0]  # 使用布尔索引提取非零元素
    #     vector_score_friend = vector_score_friend.tolist()
    #     mean_score = np.mean(vector_score_friend)
    #     mean_score_vector.append(mean_score)
    return mean_score_vector


def create_2d_diff_list(t_coo_matrix,t_diff_list,t_evo_times,z_t):
    diff_1d_evo_list = []

    grades_arr_sum_f_t = np.zeros(num_nodes, dtype=int)  # 当场计算t_coo_matrix下的标准分数score_standard，
    for j11 in range(num_nodes):
        sum_f = np.sum(col_index_to_grade(t_coo_matrix, j11))  # j11这个人的朋友的分数求和
        grades_arr_sum_f_t[j11] = sum_f
    score_standard = z_t * grades_arr + (1 - z_t) * grades_arr_sum_f_t

    '''
    score_nine_indices_t = np.where(grades_arr == 9)
    for i in range(len(score_nine_indices_t[0])):  # 用到所有的9
        nine_index_t = score_nine_indices_t[0][i]
        pre_vector_score_friend_t = np.zeros(num_nodes, dtype=int)
        for col1 in t_coo_matrix.col[t_coo_matrix.row == nine_index_t]:  # 仿照col_index_to_grade函数
        ...
        vector_score_friend = vector_score_friend.tolist() 
        diff_vector = [grades_arr[nine_index_t] - x for x in vector_score_friend]
        max_diff_value = max(diff_vector)
        diff_1d_evo_list.append(max_diff_value)
    '''
    score_nine_indices_t = np.where(grades_arr == 9)
    ...
    if t_evo_times == 1:
        for diff_value in diff_1d_evo_list:# 向diff_list添加四行，每行的第一个元素来自new_values
            t_diff_list.append([diff_value])
    else:
        for i_t, diff_value in enumerate(diff_1d_evo_list):       # 将new_values_2的值分别添加到diff_list每一行的第二列
            t_diff_list[i_t].append(diff_value)
...
for i2 in range(len(sorted_indices)):
    if (9 <grades_arr[sorted_indices[i2]]) and (grades_arr[sorted_indices[i2]] <= 10):
        indices_t = np.append(indices_t, sorted_indices[i2])
        arr_num[0] += 1
    elif (4 <grades_arr[sorted_indices[i2]]) & (grades_arr[sorted_indices[i2]] <= 5):
        indices_t = np.append(indices_t, sorted_indices[i2])
        arr_num[1] += 1
    elif (0 <grades_arr[sorted_indices[i2]]) & (grades_arr[sorted_indices[i2]] <= 1):
        indices_t = np.append(indices_t, sorted_indices[i2])
        arr_num[2] += 1
grades_sum_f_chart = np.zeros(len(indices_t), dtype=float)
grades_arr_sum_f = np.zeros(num_nodes, dtype=float)
...
def coarse_evo(t3):
    for t1 in range(t3):
        global evo_times, grades_arr_sum_f, grades_sum_f_chart
        evo_times += 1
        print(f"coarse evo {evo_times}")
        for i1 in range(num_nodes):
            sum_f = np.sum(col_index_to_grade(adj_matrix_coo, i1))  # i1这个人的朋友的分数求和
            grades_arr_sum_f[i1] = sum_f

        grades_sum_f_chart = np.vstack((grades_sum_f_chart, grades_arr_sum_f[indices_t][np.newaxis, :]))
        ...
        for high_g_node in sorted_indices:
            if lack_of_f_matrix[high_g_node] < 0:
                ...
                for high_g_in_drags in sorted_col_index_of_lack_m:  # 在缺失朋友的人中从分数最高的人开始，为他补齐朋友，因为分数高的人会先成为high_g_node
                    if lack_of_f_matrix[high_g_in_drags] < 0:  # 校验他的确还缺少朋友
                        ...
                        for t_times in range(lacking_f_num_real):
                            ...
                            while len(f_choices_temp) > 0:
                                random_f_index = np.random.choice(f_choices_temp)  # 随机选一个
                                if get_coo_value(adj_matrix_coo, high_g_in_drags,
                                                 random_f_index) == 0:  # 如果随机到的是陌生人，则停止
                                    break
                                else:
                                    f_choices_temp = np.delete(f_choices_temp,
                                                               np.where(f_choices_temp == random_f_index))
                    ...
                    # print(f"there are {drag_count_fun(lack_of_f_matrix)} people that are still lack of friends")
                    #     if flag < 1:
                    #         print(f"the first high_g_in_drags: number {high_g_in_drags} need {lacking_f_num} friends, and lacking_f_num_real is {lacking_f_num_real}")
                    #         print(f"check: after helping ,lack_of_f_matrix of number {high_g_in_drags} is {lack_of_f_matrix[high_g_in_drags]}")
                    #         flag += 1


'''
def col_index_to_sumf_grade(t_coo_matrix, t_row_number):  # 此函数会把为1的元素映射成他们的sumf分数
    t_arr1d_for_sort_g = np.zeros(num_nodes, dtype=int)
    for col in t_coo_matrix.col[t_coo_matrix.row == t_row_number]:
        t_arr1d_for_sort_g[col] = grades_arr_sum_f[col]
    return t_arr1d_for_sort_g


def drop_the_lowest_sum_f_g(t_coo_matrix, t_row_number):  # 这个函数把接收到的这个行标的朋友里分数最低的那个人drop掉
    each_f_grade_list = col_index_to_sumf_grade(t_coo_matrix, t_row_number)
    each_f_nonzero_count = len(t_coo_matrix.data[t_coo_matrix.row == t_row_number])  # 这是t_row_number现有的朋友的数量
    drop_col_index = np.argsort(each_f_grade_list)[
                     -each_f_nonzero_count:]  # 这是一个分数从低高到排序后返回的列索引，只保留倒数each_f_nonzero_count个
    f_be_dropped_col_num = drop_col_index[0]  # 分数最低的那个倒霉蛋
...
def drop_the_lowest_sum_f_g(t_coo_matrix, t_row_number, grades_arr_standard):  # 这个函数把接收到的这个行标的朋友里分数最低的那个人drop掉
...
# print("the following is refining process:")

# 精细演化，这次演化里让他们按照sum_f的高低来选择preference


grades_arr_standard = np.zeros(num_nodes, dtype=int)
pre_cc_node_nine = []
pre_cc_node_five = []
pre_cc_node_one = []
pre_cc_network = []
vector_list_2d = []
vector_list_2d.append(coo_to_friend_vector(adj_matrix_coo_ini))
diff_list = []
num_connected_list.append(adj_to_connected_c(adj_matrix_coo_ini))
def refine_evo(z_t):
    global evo_times, grades_arr_standard, adj_matrix_coo, grades_sum_f_chart, num_connected_list
    # for t2 in range(t4):
    flag = True
    while flag:
        evo_times += 1
        print(f"refine evo {evo_times}")

        cc_node_nine, cc_node_five, cc_node_one, cc_network = fun_pre_cc(adj_matrix_coo)
        pre_cc_node_nine.append(cc_node_nine)
        pre_cc_node_five.append(cc_node_five)
        pre_cc_node_one.append(cc_node_one)
        pre_cc_network.append(cc_network)# 向上几行的处理均只用来画图
        # create_2d_diff_list(adj_matrix_coo, diff_list, evo_times, z_t)
        ...
        for i1 in range(num_nodes):
            ...
            if count_f > 0:
                sum_f = np.sum(col_index_to_grade(adj_matrix_coo, i1))  # i1这个人的朋友的分数求和
                grades_arr_sum_f[i1] = float(sum_f/count_f)
        grades_arr_standard = z_t * grades_arr + (1 - z_t) * grades_arr_sum_f  # 每一个人朋友的分数总和的评价+他本人的分数，构成他的标准分数
        ...
        for high_g_node in sorted_indices_standard:
            ...
            for t_col in range(nonzero_count1):  # high_g_node遍历了他的choice_adj_coo里的所有人
                if new_degree < degree:
                    ...
                    if sorted_col_index_sum_f[t_col] in adj_matrix_coo.col[adj_matrix_coo.row == high_g_node]:
                        ...
                        if np.any(grades_arr_standard[high_g_node] > pre_f_col_sum_f_g[
                            np.nonzero(pre_f_col_sum_f_g)]):  # 利用nonzero取出非零元素
                            drop_the_lowest_sum_f_g(adj_matrix_coo, pre_f_col, grades_arr_standard)
                            drop_the_lowest_sum_f_g(adj_matrix_coo, high_g_node,
                                                    grades_arr_standard)  # 成为朋友的两个人各自都要drop掉一个现有的朋友，始终保持朋友数量不变
                            modify_adj_m_to_one(adj_matrix_coo, high_g_node, pre_f_col)
                            modify_adj_m_to_one(adj_matrix_coo, pre_f_col, high_g_node)
                            new_degree += 1
                        else:
            ...
            arr1d_g_f_sort_of_lack_matrix = np.zeros(num_nodes, dtype=float)  # 存储列索引映成分数的一维数组，只用作排序
            for t1_col in range(num_nodes):  # 把列索引映成分数的循环
                if lack_of_f_matrix[t1_col] < 0:
        ...
        vector_list_2d.append(coo_to_friend_vector(adj_matrix_coo))
        num_connected_list.append(adj_to_connected_c(adj_matrix_coo))
        # create_2d_diff_list(adj_matrix_coo, diff_list, evo_times, z_t)
        # 获取最后两行,用来比较最后两行的元素是否相同
...
for high_g_node in sorted_indices:
    ...
    sum_f_1d_for_sort = col_index_to_sumf_grade(choice_adj_coo, high_g_node)
    nonzero_count1 = np.count_nonzero(sum_f_1d_for_sort)
    sorted_col_index_sum_f = np.argsort(sum_f_1d_for_sort)[::-1][:nonzero_count1]
    new_degree = 0
    for t_col in range(nonzero_count1):  # high_g_node遍历了他的choice_adj_coo里的所有人
...
# print(np.transpose(adj_to_grade(adj_matrix_coo_ini)))
# print(np.transpose(adj_to_grade(adj_matrix_coo)))
# print("Dense adj_matrix_coo:\n")
# a = adj_matrix_coo.toarray()
# print(a)
# aa = np.zeros((8,8),dtype=int)
# for i1 in range(8):
#     for j1 in range(8):
#         if a[i1,j1] == 1:
#             aa[i1,j1] = grades_arr[j1]
# print(aa)

# a = (csr_matrix(adj_matrix_coo) + compute_sec_adj(csr_matrix(adj_matrix_coo)))
# print(f"choice_adj_csr :\n {a.toarray()}")
...
# 打印代码执行时间
print("execute time：", round(execution_time / 60.0, 2), "mins")

adj_coo_matrix_sum = adj_matrix_coo_ini + adj_matrix_coo
# calculate_percentage(adj_coo_matrix_sum,grades_arr)

grades_sum_f_chart = grades_sum_f_chart[1:]  # 删除第一行


# 画sums直方图
def sum_standard_score(t_coo_matrix,z_t):
    # 计算当下传入的t_coo_matrix下，每一个人的所有朋友的标准分数和
    ...
    t_draw_hist_arr = np.zeros(num_nodes, dtype=int)
    for i1 in range(num_nodes):
        t_arr1d_for_sort_g = np.zeros(num_nodes, dtype=int)
        for col1 in t_coo_matrix.col[t_coo_matrix.row == i1]:  # 仿照col_index_to_grade函数
            t_arr1d_for_sort_g[col1] = score_standard[col1]  # 此处为读取score_standard里i1的朋友的标准分数
        sum_f1 = np.sum(t_arr1d_for_sort_g)
        t_draw_hist_arr[i1] = sum_f1
    return t_draw_hist_arr


# save_path1 = './save_file/sum_standard_score_array.csv'
# save_path2 = './save_file/adj_matrix_coo.npz'
# save_path3 = './save_file/sum_standard_score_array_ini.csv'
# save_path4 = './save_file/adj_matrix_coo_ini.npz'
# np.savetxt(save_path1, sum_standard_score(adj_matrix_coo,0.3), delimiter=',')
# np.savetxt(save_path3, sum_standard_score(adj_matrix_coo_ini,0.3), delimiter=',')
# save_npz(save_path2, adj_matrix_coo)
# save_npz(save_path4, adj_matrix_coo_ini)
# plot_diff(diff_list, evo_times)



# calcu_cos_sim(adj_matrix_coo,adj_matrix_coo_ini)

# print(score_arr_standard)
# print(score_arr_standard[first_score_nine_indices])

# adj_draw_network(adj_matrix_coo, evo_times,seed)

# plot_heatmap(adj_to_grade(adj_matrix_coo), grades_arr_standard)
# print(vector_list_2d)
# plot_mean_and_whiskers(vector_list_2d,evo_times)

with open('save_file/vector_list_2d_self.pkl', 'wb') as file:
    pickle.dump(vector_list_2d, file)
# plot_variance(vector_list_2d,evo_times)
# line_chart(grades_sum_f_chart, arr_num, evo_times)
# plot_box(adj_to_grade(adj_matrix_coo_ini),grades_arr, grades_arr_standard)
# plot_box(adj_to_grade(adj_matrix_coo), grades_arr, grades_arr_standard)
# plot_points(evo_times, num_connected_list)
# cc_and_variance(pre_cc_node_nine, pre_cc_node_five, pre_cc_node_one,pre_cc_network,evo_times)
# sums_histogram(sum_standard_score(adj_matrix_coo_ini,0.3))
# sums_histogram(sum_standard_score(adj_matrix_coo,0.3))
...
for i1 in range(num_nodes):
    ...
    if len_each_row == degree:
        ...
        print(f"number {i1} is lack of f, its grade is {grades_arr[i1]}")
if c == num_nodes:
