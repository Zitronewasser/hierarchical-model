import numpy as np
from statistic import show_network, plot_histogram, plot_heatmap, adj_draw_network
...
def adj_to_grade(t_adj_coo):  # 邻接矩阵转换成分数的二维数组，用于画热力图
    ...
    grade_2d_arr = np.zeros((degree, num_nodes), dtype=float)
    temp_n = 0
...
for t1 in range(4):
    for high_g_node in sorted_indices:
        if lack_of_f_matrix[high_g_node] < 0:
            ...
            arr1d_g_f_sort_of_lack_matrix = np.zeros(num_nodes, dtype=float)  # 存储lack_of_f_matrix值为负的列索引映成分数的一维数组，只用作排序
            ...
            for high_g_in_drags in sorted_col_index_of_lack_m:  # 在缺失朋友的人中从分数最高的人开始，为他补齐朋友，因为分数高的人会先成为high_g_node
                ...
                if lack_of_f_matrix[high_g_in_drags] < 0:  # 校验他的确还缺少朋友
                    lacking_f_num = np.abs(lack_of_f_matrix[high_g_in_drags])  # high_g_in_drags这个人缺失的朋友数量
                    f_choices = f_choices[1:]  # 从除去第一个人以外的范围中选择

                    if lacking_f_num <= len(f_choices):
                    ...
                    check_len = 1  # 校验drags pool里有几个人已经是他的朋友
                    acquaintance_num = 0
                    f_choices_temp = f_choices.copy()
                    print(f"before f_choices:", f_choices)
                    # 遍历drags pool，挑选分数高的陌生人成为他的朋友
                    for t1_times in range(np.count_nonzero(f_choices)):

                        if check_len <= lacking_f_num_real:
                            # 判断现在即将成为朋友的人，他不是high_g_node已经有的朋友
                            if get_coo_value(adj_matrix_coo, high_g_in_drags, f_choices[t1_times]) == 1:
                                check_len += 1
                                acquaintance_num += 1
                                continue
                            else:
                                f_index = f_choices[t1_times]  # 由于朋友没满，所以这是一次单向选择
                                modify_adj_m_to_one(adj_matrix_coo, high_g_in_drags, f_index)
                                modify_adj_m_to_one(adj_matrix_coo, f_index, high_g_in_drags)
                                lack_of_f_matrix[high_g_in_drags] += 1
                                lack_of_f_matrix[f_index] += 1  # 扶贫成功后这两个人在lack_of_f_matrix里的值要做变化
                                if lack_of_f_matrix[f_index] == 0:
                                    # 若分数位于后面的人已经被选中，且其lack_of_f_matrix的值归零,则将其剔除，f_choices是列索引
                                    f_choices = np.delete(f_choices, np.where(f_choices == f_index))
                                check_len += 1
                        else:
                            break


                    if acquaintance_num == lacking_f_num_real:
                        print(f"there is no stranger for number {high_g_in_drags} in drag pool; after f_choices:",
                              f_choices)
                    else:
...
c1 = 0
for i1 in range(num_nodes):
    row_as_array = np.squeeze(a[i1, :])  # 提取一行作为一维数组
    if np.count_nonzero(row_as_array) == degree:
        c += 1
    elif np.count_nonzero(row_as_array) > degree:
        c1 += 1
    else:
