...
def col_index_to_sumf_grade(t_coo_matrix, t_row_number, grades_arr_sum_f):  # 此函数会把为1的元素映射成他们的sumf分数
    t_arr1d_for_sort_g = np.zeros(num_nodes, dtype=int)
    for col in t_coo_matrix.col[t_coo_matrix.row == t_row_number]:
...
def drop_the_lowest_sum_f_g(t_coo_matrix, t_row_number):  # 这个函数把接收到的这个行标的朋友里分数最低的那个人drop掉
    each_f_grade_list = col_index_to_sumf_grade(t_coo_matrix, t_row_number, grades_arr_sum_f)
...
for t1 in range(15):
    ...
    grades_arr_standard = grades_arr + grades_arr_sum_f  # 每一个人朋友的分数总和+他本人的分数，构成他的标准分数
    ...
    for high_g_node in sorted_indices_standard:
        ...
        sum_f_1d_for_sort = col_index_to_sumf_grade(choice_adj_coo, high_g_node, grades_arr_sum_f)
        ...
        for t_col in range(nonzero_count1):  # high_g_node遍历了他的choice_adj_coo里的所有人
            if new_degree < degree:
                ...
                if sorted_col_index_sum_f[t_col] in adj_matrix_coo.col[adj_matrix_coo.row == high_g_node]:
                    ...
                    # 这是一次双向选择，选择者的分数要至少大于被选者朋友中任何一人的分数，没有等号表示若分数相同，默认保留旧的朋友
                    pre_f_col_sum_f_g = col_index_to_sumf_grade(adj_matrix_coo, pre_f_col, grades_arr_sum_f)
                    if np.any(grades_arr_sum_f[high_g_node] > pre_f_col_sum_f_g[np.nonzero(pre_f_col_sum_f_g)]):  # 利用nonzero取出非零元素
...
grades_arr_standard = grades_arr + grades_arr_sum_f
