import numpy as np
from statistic import show_network, plot_histogram, plot_heatmap, adj_draw_network, line_chart, z_optimize
...
# grades_arr_standard_ini = 0.1*grades_arr + 0.9*grades_arr_sum_f_ini
...
def coarse_evo(t3, adj_matrix_coo_tt, lack_of_f_matrix_tt):
    for t1 in range(t3):
        print("corse evo:", t1)
        for high_g_node in sorted_indices:
            if lack_of_f_matrix_tt[high_g_node] < 0:
                # print(f"number {high_g_node} is still lack of {np.abs(lack_of_f_matrix[high_g_node])} friends")
                # print("The process has to end")
                break
            else:

                choice_adj_csr = csr_matrix(adj_matrix_coo_tt) + compute_sec_adj(
                    csr_matrix(adj_matrix_coo_tt))  # 邻接矩阵+次级邻居
                choice_adj_coo = choice_adj_csr.tocoo()
                unsort_col_index = choice_adj_coo.col[choice_adj_coo.row == high_g_node]
                arr1d_for_sort_g = col_index_to_grade(choice_adj_coo, high_g_node)
                ...
                for t_col in range(nonzero_count_in_g):  # high_g_node遍历了他的choice_adj_coo里的所有人
                    if new_degree < degree:
                        # 判断现在即将成为朋友的人，他不是high_g_node已经有的朋友
                        if sorted_col_index[t_col] in adj_matrix_coo_tt.col[adj_matrix_coo_tt.row == high_g_node]:
                            new_degree += 1
                            continue
                        else:
                            pre_f_col = sorted_col_index[t_col]
                            # 这是一次双向选择，选择者的分数要至少大于被选者朋友中任何一人的分数，没有等号表示若分数相同，默认保留旧的朋友
                            pre_f_col_g = col_index_to_grade(adj_matrix_coo_tt, pre_f_col)
                            if np.any(grades_arr[high_g_node] > pre_f_col_g[np.nonzero(pre_f_col_g)]):
                                drop_the_lowest_g(adj_matrix_coo_tt, pre_f_col)
                                drop_the_lowest_g(adj_matrix_coo_tt, high_g_node)  # 成为朋友的两个人各自都要drop掉一个现有的朋友，始终保持朋友数量不变
                                modify_adj_m_to_one(adj_matrix_coo_tt, high_g_node, pre_f_col)
                                modify_adj_m_to_one(adj_matrix_coo_tt, pre_f_col, high_g_node)
                                new_degree += 1
                            else:
                                continue
                ...
                arr1d_g_f_sort_of_lack_matrix = np.zeros(num_nodes, dtype=int)  # 存储列索引映成分数的一维数组，只用作排序
                for t1_col in range(num_nodes):  # 把列索引映成分数的循环
                    if lack_of_f_matrix_tt[t1_col] < 0:
                        arr1d_g_f_sort_of_lack_matrix[t1_col] = grades_arr[t1_col]

                sorted_col_index_of_lack_m = np.argsort(arr1d_g_f_sort_of_lack_matrix)[::-1]  # 从高到低分数排序的列索引
                sorted_col_index_of_lack_m = sorted_col_index_of_lack_m[
                                             :np.count_nonzero(lack_of_f_matrix_tt < 0)]  # 只保留前面真正drag的列索引
                if sorted_col_index_of_lack_m.shape[0] < 2:  # 避免f_choices为空
                    # print(f"len of sorted_col_index_of_lack_m is {sorted_col_index_of_lack_m.shape[0]}, continue")
                    continue
                f_choices = sorted_col_index_of_lack_m.copy()
                ...
                flag = 0
                for high_g_in_drags in sorted_col_index_of_lack_m:  # 在缺失朋友的人中从分数最高的人开始，为他补齐朋友，因为分数高的人会先成为high_g_node
                    if lack_of_f_matrix_tt[high_g_in_drags] < 0:  # 校验他的确还缺少朋友
                        lacking_f_num = np.abs(lack_of_f_matrix_tt[high_g_in_drags])  # t2_col这个人缺失的朋友数量
                        f_choices = f_choices[1:]  # 从除去第一个人以外的范围中选择
                        if len(f_choices) == 0:  # 避免drags pool 里都是熟人而导致它为空
                            continue
                        elif lacking_f_num <= len(f_choices):
                        ...
                        for t_times in range(lacking_f_num_real):
                            ...
                            while len(f_choices_temp) > 0:
                                ...
                                if get_coo_value(adj_matrix_coo_tt, high_g_in_drags,
                            ...
                            modify_adj_m_to_one(adj_matrix_coo_tt, high_g_in_drags, random_f_index)
                            modify_adj_m_to_one(adj_matrix_coo_tt, random_f_index, high_g_in_drags)
                            lack_of_f_matrix_tt[high_g_in_drags] += 1
                            lack_of_f_matrix_tt[random_f_index] += 1  # 扶贫成功后这两个人在lack_of_f_matrix里的值要做变化
                            if lack_of_f_matrix_tt[random_f_index] == 0:
                                # 若分数位于后面的人已经被选中，且其lack_of_f_matrix的值归零,则将其剔除，f_choices是列索引
                                f_choices = np.delete(f_choices, np.where(f_choices == random_f_index))
                            # print(f"this is printed in th t_times for circle:\n", adj_matrix_coo.toarray())
    ...
    return  adj_matrix_coo_tt, lack_of_f_matrix_tt

print("\nthe following is refining process:\n")
# adj_draw_network(adj_matrix_coo)
# 精细演化，这次演化里让他们按照sum_f的高低来选择preference
z_values = np.round(np.arange(0.1, 1, 0.1), 3)  # 生成从 0.1 到 1 的步长为 0.1 的 z 值数组
evo_times_arr = np.array([], dtype=int)
grades_sum_f_chart_last_row = np.zeros(len(indices_t), dtype=int)
...
for z in z_values:
    ...
    lack_of_f_matrix = np.zeros(num_nodes, dtype=int)
    adj_matrix_coo = adj_matrix_coo_ini.copy()

    # 是否进行粗演化
    # adj_matrix_coo, lack_of_f_matrix = coarse_evo(0, adj_matrix_coo, lack_of_f_matrix)
    ...
    while flag:
        ...
        grades_arr_standard = z * grades_arr + (1 - z) * grades_arr_sum_f  # 每一个人朋友的分数总和+他本人的分数，构成他的标准分数
        ...
        sum_f_iso_check = np.zeros(num_nodes, dtype=int)
        for i1 in range(num_nodes):
            sum_f = np.sum(col_index_to_grade(adj_matrix_coo, i1))  # i1这个人的朋友的分数求和
            sum_f_iso_check[i1] = sum_f
        sum_f_iso_check = grades_arr + sum_f_iso_check
        ios_result = np.any(grades_arr == sum_f_iso_check)  # 如果有孤立节点，说明他没有朋友
        if ios_result:
            evo_times = 99
            break
        elif evo_times > 99:
    ...
    evo_times_arr = np.append(evo_times_arr, evo_times)
    grades_sum_f_chart_last_row = np.vstack((grades_sum_f_chart_last_row, grades_sum_f_chart[-1][np.newaxis, :]))
...
'''
# grades_arr_standard = 0.1*grades_arr + 0.9*grades_arr_sum_f
...
# print(grades_sum_f_chart)
grades_sum_f_chart_last_row = grades_sum_f_chart_last_row[1:]
z_optimize(grades_sum_f_chart_last_row, arr_num, evo_times_arr, z_values)
